...
import com.codeops.logger.config.RequestCorrelationFilter;
...
/**
 * Configures Spring Security for the CodeOps-Logger API with stateless JWT-based authentication.
 *
 ...
 *   <li>{@link RequestCorrelationFilter} and {@link JwtAuthFilter} are registered before
 *       {@link UsernamePasswordAuthenticationFilter}</li>
 * </ul>
 *
 * @see JwtAuthFilter
 * @see com.codeops.logger.config.CorsConfig
 */
@Configuration
@EnableWebSecurity
...
public class SecurityConfig {
     ...
     * CORS support, and security response headers.
     *
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                // CSRF disabled: stateless JWT API with no cookie-based auth
                .csrf(csrf -> csrf.disable())
                .cors(cors -> cors.configurationSource(corsConfigurationSource))
                .sessionManagement(sm -> sm.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/api/v1/logger/health").permitAll()
