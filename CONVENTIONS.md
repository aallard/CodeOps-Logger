...
## CodeOps-Logger Specific Conventions

- **Port:** 8098 (application), 5437 (PostgreSQL)
- **Package:** `com.codeops.logger`
- **API Prefix:** `/api/v1/logger/`
- **JWT:** Validate only — never issue tokens. Shared secret with CodeOps-Server.
- **Kafka:** Connects to existing broker at localhost:9094. Consumer group: `codeops-logger`.
...
- **Exception hierarchy:** LoggerException -> NotFoundException, ValidationException, AuthorizationException.
